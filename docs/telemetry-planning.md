...
- [ ] **Create telemetry package** (`telemetry/init.go`, `ctx.go`, `meter.go`).
- [ ] Add dependencies to `go.mod` & `go.sum`.
- [ ] **Phase 1** – wire `main.go` bootstrap, expose `/metrics`.
- [ ] **Phase 2** – instrument `DeltaWebsocketClient.readPump` (span + counters).
